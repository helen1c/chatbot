...
def load_model(model, checkpoint, args, verbose=False):
    ...
    if checkpoint is None or checkpoint == "None":
        ...
        if hasattr(model, "transformer") and all(
            not s.startswith("transformer.") for s in model_state_dict.keys()
        ):
...
def get_eval_list_same_length(input_file, tokenizer, max_batch_size, norm=True):
    ...
    def batch_feature_same_len(features):
        input_ids = torch.stack(
            [
                torch.tensor(f.choices_features["input_ids"], dtype=torch.long)
                for f in features
            ]
        )
        ...
        token_type_ids = torch.stack(
            [
                torch.tensor(f.choices_features["token_type_ids"], dtype=torch.long)
                for f in features
            ]
        )
        ...
        response_len = torch.tensor(
            [f.response_len for f in features], dtype=torch.long
        )
        return (
            input_ids,
            position_ids,
    ...
    for l in sorted(dataloader_pre):
        ...
        if len(f[0]) <= max_batch_size:
            ...
            while True:
                dataloader.append(
                    [ff[start_index : start_index + max_batch_size] for ff in f]
                )
...
def set_lr(
    optimizer, step, schedule, lr, warmup_steps, warmup_proportion, n_embd, tot_steps
):
    if schedule == "None":
        lr_this_step = lr
        ...
        lr_this_step = lr * warmup_linear(step / tot_steps, warmup_proportion)
